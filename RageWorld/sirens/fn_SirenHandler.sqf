...
// just some vars you do not need to bother about...
/*if(isNil{_veh getVariable"xcar"})then{
	if(isNil{_veh getVariable"Blight"})exitWith{};
};*/ 	
...
if(_status)then{
	if(_oldSiren == _siren)then{
		...
		_veh setVariable["RageSiren",[false,"",0],true];			// 0 is false when the siren should turn off in a moment
	}else{
		_veh setVariable["RageSiren",[true,_siren,_time],true];		// 0 is still true  script will keep on going but with new siren to play..
	};	
}else{	
